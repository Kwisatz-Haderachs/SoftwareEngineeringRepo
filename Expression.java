...
public class Expression {
    ...
    public Double evaluate(){
        ...
        while (mTokenQueue.isEmpty()==false){
            ...
            if(token instanceof Operand){
                ...
                LeftParen leftparen = (LeftParen)token;
    ...
    public void topEval(Stack<Operator> pOperatorStack, Stack<Operand> pOperandStack){
        ...
        if(operator instanceof UnaryOperator){
            Operand ret = ((UnaryOperator)operator).evaluate(right);
            pOperandStack.push(ret);

        } else {
            Operand left = pOperandStack.pop();
            Operand rez = ((BinaryOperator)operator).evaluate(left, right);
            pOperandStack.push(rez);
        }
    }
